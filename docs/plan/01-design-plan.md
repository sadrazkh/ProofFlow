...
> **انحراف آگاهانه هنگام اجرا:** Checkbox همیشه‌حاضر به‌جای دکمهٔ Hover. کنترلی که فقط با Hover
> ظاهر می‌شود روی لمس اصلاً وجود ندارد و با کیبورد هم پیدا نمی‌شود — و این تصمیمِ «کدام فیلد وارد
> نسخهٔ بعد شود» است، نه یک میان‌بر. متن نوار پایینی «۰ پذیرفته، ۳ باقی‌مانده» است؛ شمارهٔ نسخهٔ
> بعدی را نمی‌گوید، چون سرور آن را تعیین می‌کند و حدس‌زدنش در مرورگر می‌تواند غلط از آب دربیاید.

> **افزودهٔ حین اجرا:** دکمهٔ «افزودن به‌عنوان قاعده» زیر فهرست پیشنهادها. رایج‌ترین مقایسه آن است
> که همهٔ تفاوت‌ها فیلدِ خودبه‌خود متغیر باشند؛ پاسخ درست سه قاعده است و هیچ نسخهٔ تازه‌ای — و بدون
> این دکمه تنها راهِ ثبت پیشنهادها «ذخیره به‌عنوان نسخهٔ تازه» بود، که یک timestamp را به‌عنوان
> مقدار Baseline تبرک می‌کند.
...
> **وضعیت اجرا (فاز H):** هر دو انجام شد. Cron خام در یک chip تک‌فاصله کنار ترجمهٔ انسانی و نام
> منطقهٔ زمانی می‌آید — ترجمه راحتی است و عبارت حقیقت، و کسی که باید تغییرش دهد باید هر دو را ببیند.
> فقط عبارت‌هایی ترجمه می‌شوند که واقعاً شناخته شده‌اند؛ یک مترجم عمومی cron موارد جالب را
> نامحسوس اشتباه می‌گوید. «اجرای بعدی» به کمکی رو به جلو نیاز داشت: `RelativeTime` عمداً زمان آینده
> را «همین حالا» می‌خواند، که روی این ستون دروغ بود.
...
- [x] **D-H — انجام شد** (زمان‌بندی با Cron + ترجمه + «بعدی»، ناپایداری و Quarantine)
- [x] **D-I — انجام شد** (صفحه تیم با کارت نقش‌ها و Confirm شمارشی، صندوق تأیید، فیلترهای Audit، بازیابی گذرواژه با SMTP)
- [x] **D-J — انجام شد** (گالری ۱۲ الگو با پیش‌نمایش تولیدشده از خود گراف، Import سه‌مرحله‌ای با شمارش، هشدار صریح Secret در Export)
- [x] **D-K — انجام شد** (چیپ «redacted» با Tooltip در تنها جایی که هر مقدار JSON رندر می‌شود، صفحهٔ Runner با کد بزرگ و Countdown و پنج وضعیت به‌صورت نقطه+کلمه، و انتخاب Runner روی خودِ محیط)
- [x] **D-L — انجام شد** (اجرا از یک Node، نام‌گذاری سناریو، Onboarding چهارگامی، رفتار Dialog با کیبورد، و اعداد واقعی داشبورد به‌جای صفرهای دروغین)
- [x] **D-M — انجام شد** (Docker یک‌کانتینری با راستی‌آزمایی در CI، Backup/Restore تأییدشده روی داده واقعی شامل رمزگشایی Secret، و مسیر ارتقا با دیتابیس ساخته‌شده توسط نسخهٔ قبلی)
