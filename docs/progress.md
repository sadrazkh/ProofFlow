...
| Getting a token to test with | An authorisation panel beside the request: bearer, basic, an API key in a header, or OAuth 2 by client credentials or by username and password. «Get a token» asks the token endpoint — through the same URL guard as everything else — and every request afterwards carries it as a header that shows up under «what was sent» |
| Credentials that are not typed on a shared screen | Every box in that panel resolves references, so a client secret can be `{{secrets.clientSecret}}`. What is remembered between visits is the shape — the address, the client id, the scope — and never the secret, the password or the token |
| Bringing a Postman collection over | The importer has read collections since Phase J and nobody found it. There is a button on the request page and on the scenarios page now, pointing at it |
